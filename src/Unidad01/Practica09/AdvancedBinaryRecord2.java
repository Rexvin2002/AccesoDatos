...
public class AdvancedBinaryRecord2 {
    ...
    public AdvancedBinaryRecord2(String path, List<String> f, List<Integer> fl) throws IOException {
        this.records = new Record(f, fl);
    ...
    public Map<String, String> read(long position) {
        ...
        try (RandomAccessFile rndFile = new RandomAccessFile(this.file, "r")) {
            rndFile.seek(position * records.getRecordLength()); // Move to the specified position.

            // Read the record from the file
            for (int i = 0; i < records.getFieldsCount(); i++) {
                String field = records.getField(i);
                int length = records.getFieldLength(i);
    ...
    public static void main(String[] args) {
        try {
            ...
            AdvancedBinaryRecord2 recordManager = new AdvancedBinaryRecord2("records.dat", fields, fieldLengths);

            // Crear un nuevo registro como un Map
            Map<String, String> newRecord = new HashMap<>();
            newRecord.put("ID", "22222222B");
            newRecord.put("NAME", "Nombre2");
            newRecord.put("DIRECTION", "Calle Principal Nº 7, Planta4");
            newRecord.put("ZC", "12345");

            // Añadir el nuevo registro
            recordManager.insert(newRecord, 0); // Insertar en la posición 0

            // Leer y mostrar el registro almacenado
            Map<String, String> readRecord = recordManager.read(0);
...
/*
    ...
    public List<String> selectColumn(String columnName) {
        ...
        int colIndex = records.getFields().indexOf(columnName); // Get the index of the column.
        if (colIndex == -1) {
    ...
    public List<String> selectAllValuesForColumn(String columnName) {
        List<String> values = new ArrayList<>(); 
        int totalRecords = records.getRecordCount(); 
    ...
    public void delete(int row) {
        try (RandomAccessFile rndFile = new RandomAccessFile(this.file, "rw")) {
            rndFile.seek(row * this.records.getRecordLength()); // Move to the specified position.
            byte[] emptyBytes = new byte[(int) this.records.getRecordLength()]; // Create empty bytes to clear the record.
